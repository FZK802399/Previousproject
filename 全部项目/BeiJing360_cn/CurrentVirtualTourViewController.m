...
- (void)viewDidLoad {
	...
	if ([MySingleton sharedSingleton].currentFlagWithVirtualTour == 1) {
		...
		if (![MySingleton sharedSingleton].morePanoramoURLString) {
			NSLog(@"ok");
		}
		[self.webview loadRequest:[NSURLRequest requestWithURL:
								   [NSURL URLWithString:[MySingleton sharedSingleton].morePanoramoURLString]]];
	} else if ([MySingleton sharedSingleton].currentFlagWithVirtualTour == 3) {

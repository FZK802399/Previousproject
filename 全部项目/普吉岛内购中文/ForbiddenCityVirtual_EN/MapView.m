...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        CGRect rc = CGRectMake(kLCDH - 528 - 125, kLCDW - 82.0f - 561 - 4 - 20, 491, 561);
        CGRect maskRC = CGRectMake(0, 0, 491, 561);
        self.frame = rc;
        ...
        _mapView = [[MKMapView alloc] initWithFrame:CGRectMake(0, 0, 491, 561 - 15)];
        [_mapView setDelegate:self];
                
        CLLocationCoordinate2D mapCenter;
        mapCenter.longitude = 98.293782;
        mapCenter.latitude = 8.005697;
        ...
        _locatButton = [[UIButton alloc] initWithFrame:CGRectMake(491 - 70, 561 - 86, 64, 64)];
        ...
        _changeButton = [[UIButton alloc] initWithFrame:CGRectMake(491 - 70 - 74, 561 - 86, 64, 64)];
...
- (void)loadSceenWithTag:(NSNumber *)tag
{
    ...
    {
        if (!wasEnglishPaid && tag.integerValue > kFreeSceneCount)
        {
            [_delegate performSelector:@selector(showPaymentView)];
        }
        else
        {
    ...
    {
        if (!wasChinesePaid && tag.integerValue > kFreeSceneCount)
        {
            [_delegate performSelector:@selector(showPaymentView)];
        }
        else
...
- (void)updateMapAfterPaymentSuccess
{
    [_mapView removeAnnotations:_mapView.annotations];
    [self addAnnotationWithSceneInfos:_pointsInfo];
}

- (void)updateMapAfterPaymentSuccessWithSceneInfos:(NSArray *)sceneInfos
{
    [_mapView removeAnnotations:_mapView.annotations];
    [self addAnnotationWithSceneInfos:sceneInfos];
}
...
- (void)displaySysMapAfterCurrentSceneChange
{
    for (Annotation * ann in _mapView.annotations)
    {
        ...
        if (ann.identifier == _currentSceneTag)
        {
            ...
            id objc = [_mapView.annotations objectAtIndex:_mapView.annotations.count - 1];
            if (![objc isKindOfClass:[MKUserLocation class]])
            {
                return;
            }

...
@property (strong, nonatomic) DXMessageToolBar *chatToolBar; //聊天框
...
- (void)sendCommodityMessageWithInfo:(NSDictionary *)info
{
    ...
    EMMessage *tempMessage = [ChatSendHelper sendTextMessageWithString:@"商品信息" toUsername:_conversation.chatter isChatGroup:NO requireEncryption:NO ext:extDic];
    [self addMessage:tempMessage];
}
...
- (NSDictionary*)getUserInfoAttribute
{
    ...
    if (![config isLogin]) {
        ...
        iOSName = [NSString stringWithFormat:@"%d",i];
        mobile = @"123";
    }
    else{
        iOSName = config.loginName;
        mobile = config.Mobile ? config.Mobile : config.loginName;
    }
    NSDictionary *ext = nil;
    NSMutableDictionary *visitor = [NSMutableDictionary dictionary];
    [visitor setObject:iOSName forKey:@"trueName"];
    [visitor setObject:@"10000" forKey:@"qq"];
    [visitor setObject:mobile forKey:@"phone"];
    [visitor setObject:@"efood7" forKey:@"companyName"];
    [visitor setObject:iOSName forKey:@"userNickname"];
    [visitor setObject:@"abc@123.com" forKey:@"email"];
    switch (_saleType) {
        case ePreSaleType:
            ext = @{@"visitor":visitor,@"queueName":@"客服"};
            break;
        case eAfterSaleType:
            ext = @{@"visitor":visitor,@"queueName":@"客服"};
            break;

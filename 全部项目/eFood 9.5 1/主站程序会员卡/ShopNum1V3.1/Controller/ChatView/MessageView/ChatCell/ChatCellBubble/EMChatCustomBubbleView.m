...
- (id)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame])
    {
        ...
        _cimageView.contentMode = UIViewContentModeScaleAspectFit;
...
- (void)setModel:(MessageModel *)model
{
    ...
    if ([[model.message.ext objectForKey:@"msgtype"] isKindOfClass:[NSDictionary class]]) {
        NSDictionary * dict1 = [model.message.ext objectForKey:@"msgtype"];
        if ([[dict1 objectForKey:@"track"] isKindOfClass:[NSDictionary class]]) {
            NSDictionary * dict2 = [dict1 objectForKey:@"track"];
            NSString * imageUrl = [dict2 objectForKey:@"img_url"];
            if (imageUrl.length > 0) {
                [_cimageView setImageWithURL:[NSURL URLWithString:imageUrl] placeholderImage:[UIImage imageNamed:@"imageDownloadFail.png"]];
            }
        }
    }

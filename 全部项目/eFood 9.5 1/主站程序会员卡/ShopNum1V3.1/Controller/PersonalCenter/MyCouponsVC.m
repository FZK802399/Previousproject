...
@interface MyCouponsVC ()<UIScrollViewDelegate,CanMakeCouponsViewControllerDelegate>
...
- (void)viewDidLoad {
    ...
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc]initWithTitle:@"使用详情" style:UIBarButtonItemStylePlain target:self action:@selector(UseDetails)];
}


-(void)setupChilds
{
//    CanMakeCouponsViewController *canvc = [[CanMakeCouponsViewController alloc]init];
//    canvc.aDelegate = self;
//    [self addChildViewController:canvc];
    [self addChildViewController:[[CanMakeCouponsViewController alloc]init]];
    [self addChildViewController:[[noMakeCVC alloc]init]];
...
- (void)setupTitleButtons
{
  ...
  //NSArray *titles = @[@"可使用优惠券(10)", @"不可使用优惠券(6)"];

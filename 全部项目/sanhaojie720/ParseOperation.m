...
//		self.elementsToParse = [NSArray arrayWithObjects:kNameStr, kImageStr, kArtistStr, nil];
		...
		if (self.workingEntry)
			{
				if (storingCharacterData)
				{
					...
					{
						self.workingEntry.appID = trimmedString;
					//	NSLog(@"appID=%@", self.workingEntry.appID);
					}
					else if ([elementName isEqualToString:kNameStr])
					{   
						self.workingEntry.appName = trimmedString;
					//	NSLog(@"appName=%@", self.workingEntry.appName);
					}
					else if ([elementName isEqualToString:kImageStr])
					{
						self.workingEntry.imageURLString = [NSString stringWithFormat:@"http://%@/sanhaojie_vtour/%@", WEBADDRESS, trimmedString];
						NSLog(@"appName=%@", self.workingEntry.imageURLString);
					}
